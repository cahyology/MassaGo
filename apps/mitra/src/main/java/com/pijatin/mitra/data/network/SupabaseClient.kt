package com.pijatin.mitra.data.network
...
class SupabaseClient(
    ...
    suspend fun acceptOrder(orderId: String, therapistId: String, therapistLat: Double = 0.0, therapistLng: Double = 0.0): Boolean = withContext(Dispatchers.IO) {
        try {
            val profile = com.pijatin.mitra.data.repository.TherapistRepository.instance.therapistProfile.value
    ...
    suspend fun updateTherapistGpsForOrder(orderId: String, lat: Double, lng: Double): Boolean = withContext(Dispatchers.IO) {
        try {
            val therapistId = com.pijatin.mitra.data.repository.TherapistRepository.instance.therapistProfile.value.id
            updateLocationAndDuty(therapistId, lat, lng, isOnline = true)
            true
        } catch (e: Exception) {
            e.printStackTrace()
        ...
        try {
            ...
            val payload = JsonObject().apply {
                ...
                add("customer", JsonObject().apply {
                    ...
                    addProperty("name", therapistName.ifBlank { "Mitra PijatIn" })
